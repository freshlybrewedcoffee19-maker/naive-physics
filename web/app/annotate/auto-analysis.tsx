...
import { acceptedLabel, enforceEpisodeStateBoundaries, generateTemporalCandidates, TEMPORAL_TRACKS, type CandidateLabel, type TemporalCandidate, type TemporalLabel } from "./temporal-candidates";
...
export function AutoAnalysis({ videoRef, episodeId, fps, currentTime, duration, dimensions, activeRegion, temporal, onAcceptTemporal }: { videoRef: RefObject<HTMLVideoElement | null>; episodeId: string; fps: number | null; currentTime: number; duration: number; dimensions: { width: number; height: number }; activeRegion: string; temporal: TemporalRecord[]; onAcceptTemporal: (candidate: { label: TemporalLabel; start_time_sec: number; end_time_sec: number }) => boolean }) {
  ...
  const runAnalysis = async () => {
    ...
    setRunning(true); setAnalysisReady(false); setNotice(`${mode} analysis running. Temporal annotation remains independent.`);
  ...
  const generateCandidates = () => {
    ...
    const generated = generateTemporalCandidates({ episodeId, duration, fps, hands, tools: toolTracks, toolOnGarmentTimes:semantics.filter((item)=>item.semantic_label==="iron_contact_point").map((item)=>item.timestamp_sec), slips: slips.filter((item)=>item.review_status!=="rejected"), rgb: rgbScores, garmentMaskAvailable:Boolean(mask) });
    setTemporalCandidates(generated); setAdjustingCandidate(null); setNotice(`${generated.length} temporal candidates generated for review. No annotation was accepted automatically.`);
  };
  ...
  return <section className={styles.autoAnalysis} aria-labelledby="auto-analysis-title">
    ...
    <section className={styles.temporalAutomation} aria-labelledby="temporal-review-title"><div className={styles.analysisSubhead}><div><p>INTERPRETABLE TEMPORAL PROPOSALS</p><h3 id="temporal-review-title">Temporal review</h3><span>Automation proposes intervals. Only an explicit human acceptance adds a verified annotation.</span></div><button disabled={!analysisReady || !fps} onClick={generateCandidates} type="button">GENERATE TEMPORAL CANDIDATES</button></div>
